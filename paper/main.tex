...
against its own reference implementations: of the 44 designs whose unmodified reference passes
baseline validation, 73\% fall below the 95\% floor our own suite is held to, three score a 0\%
kill rate outright, and one design's own shipped golden reference fails its own shipped
testbench. Where a comparable audit of NVIDIA's CVDP benchmark is
...
suite is certified against, with three designs at a 0\% kill rate and one design's own shipped
golden reference failing its own shipped testbench --- plus a documented structural finding,
...
\texttt{41b26896e33b5} fetched on 2026-07-29. Of the 50 designs in RTLLM v2.0, 44 passed baseline
validation and were audited; 6 were reported \texttt{unsupported}. Four of the six are genuine
...
by RTLLM's own shipped testbench. At the other extreme, 12 of the 44 designs do reach a 100\%
kill rate, so the result is not that RTLLM's testbenches are uniformly weak, but that their rigor
is highly uneven and, for a majority of designs, well short of what a mutation-kill floor would
require. A deterministic 20\% re-run sample (8 of the 44 eligible designs, fixed seed
\texttt{20260729}) reproduced byte-identical per-design results across two independent runs,
certifying the measurement.
