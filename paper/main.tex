...
\paragraph{Failure-stage taxonomy, and a genuine extraction bug we found while producing it.} We did not originally report which stage each failing submission fails at; assembling that breakdown while responding to review revealed a result significant enough to report in full rather than fold into a single sentence. Across all 209 failing (model, task) pairs in the Track A campaign, 85.6\% fail at Stage 0 (lint), 13.4\% at Stage 1 (simulation), and 1.0\% at Stage 2 (formal); essentially none of the campaign's failures are attributable to a design that compiles and simulates but fails a formal property. Inspecting the Stage-0 lint logs directly, 58.9\% of lint failures involve no genuine syntax or semantic error at all: Verilator's default policy escalates certain classes of warning (predominantly implicit bit-width truncation/expansion, \texttt{WIDTHTRUNC}/\texttt{WIDTHEXPAND}) to a fatal exit, and our lint invocation (\texttt{verilator --lint-only --sv}) applies no waivers, so a submission that would simulate correctly can still score zero for a width-safety style issue. More seriously, we found that 35 of Gemini 2.5 Pro's 44 failing submissions (all 35 scoring exactly zero) begin with an unstripped Markdown code-fence marker (e.g.\ a literal \texttt{\`{}\`{}\`{}systemverilog} as the file's first line), which Verilator parses as an undefined preprocessor directive and rejects outright, independent of and prior to any assessment of the RTL itself. This is a genuine extraction-pipeline bug, not a finding about the model: it means Gemini 2.5 Pro's reported 16/60 pass@1 --- the lowest of the seven models evaluated, and the basis for several comparative claims in Section~\ref{sec:results} --- is a substantial undercount of its actual capability on this suite, and we cannot currently say by how much without re-extracting and re-scoring those 35 submissions. We are disclosing this rather than silently re-running the campaign to produce a more favorable number before submission; the fix and re-run are necessary before Gemini 2.5 Pro's leaderboard position, or any claim in this paper that depends on it, should be treated as final. Separately, GPT-5 and GPT-5-mini produced no extractable submission at all (as opposed to a submission that failed a gate) for 22/60 and 11/60 tasks respectively, heavily concentrated on the same Tier-2 and Tier-3 tasks for both models, consistent with a reasoning-token-budget interaction on longer specifications rather than a shared extraction defect; we have not root-caused this further.
