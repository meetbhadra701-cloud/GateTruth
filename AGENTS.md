# SiliconBench â€” Implementer Instructions
...
## Acceptance & evidence protocol (anti-fake-work â€” non-negotiable)
...
- **Determinism**: pin every tool invocation/seed. Same input â†’ byte-identical manifest except
...
- **References & hidden vectors are human-signed-off only.** Never author or alter `ref/ref.sv` logic or
  hidden vectors from your own knowledge â€” if an Architect draft is missing, write a blocker (DO-NOT-BUILD rule 9).
- **Never** put private strategy, evidence, or non-public planning material in this public repo.
- **Banned**: stubbing/xfail-ing tests to pass Â· loosening timeouts Â· editing acceptance criteria Â·
  mocking tool output Â· claiming success without pasted output Â· committing to `main`.
