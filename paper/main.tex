...
95\% floor our own suite is held to, and three (one of them a single-generic-mutant design, so its
0\% should be weighted accordingly) score a 0\%
...
flag: \texttt{asyn\_fifo}'s
testbench uses a SystemVerilog \texttt{break} statement, \texttt{ring\_counter}'s testbench requires
a SystemVerilog whole-array assignment, \texttt{clkgenerator} and
\texttt{freq\_divbyodd} each drive a \texttt{reg} from a continuous assignment in a way Icarus
rejects under this flag, and \texttt{multi\_8bit}'s reference file contains a \texttt{for}-loop syntax
error. We did not re-run these five under a newer Icarus generation flag (e.g.\ \texttt{-g2012}),
which supports more SystemVerilog constructs and might admit some of them; we report the exact flag
used rather than the broadest one available, and leave that re-check as future work
(Section~\ref{sec:limitations}) rather than silently re-running under looser semantics after the
fact. The sixth, and the only one of the six we treat as a consequential finding in its own right
rather than a tooling gap, is
\texttt{radix2\_div}: its \emph{unmodified golden reference fails its own shipped
testbench} (three of its self-checking vectors report a mismatch), independent of anything
GateTruth's mutation engine or generation flag does.
