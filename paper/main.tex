...
actually perform against this suite? We present the empirical results of the v1.0 baseline evaluation campaign, run 2026-07-22, encompassing seven models evaluated in that campaign (not necessarily each provider's current frontier at time of reading): three from Anthropic (Claude Opus 4.8, Sonnet 4.6, Haiku 4.5), two from OpenAI (GPT-5, GPT-5-mini), and two accessed via OpenRouter (Google Gemini 2.5 Pro, Meta Llama 4 Maverick). These models were evaluated across the complete suite of 60 Track A tasks and 8 Track B tasks. All evaluations utilized a single-sample inference protocol at a temperature of zero, substituting provider-default sampling exclusively for reasoning models that do not support explicit temperature parameterization. Executions were strictly confined within a singular, content-addressed container environment (\texttt{gatetruth:v1}, digest recorded per manifest) under a \$300 per-run spend cap, and scored exclusively against maintainer-reviewed reference RTL and held-out hidden vectors. Accounting for both tracks across all seven models, and reconciled against the settled provider billing ledger (Anthropic \$18.79, OpenAI \$14.54, OpenRouter \$17.75), the campaign's total metered cost was \$51.08; this reconciled total includes transient-transport retries and is not directly derivable by summing the per-model costs in Tables~\ref{tab:tracka}--\ref{tab:trackb}, which report only the cost of each model's accepted official sample. This \$51.08 figure covers only the 2026-07-22 v1.0 campaign itself; the Appendix~\ref{app:variance} run-to-run variance study is a separate, later evaluation (2026-07-26) that cost an additional \$8.18 and is not included in this total. A third, separate expenditure is the single-model token-budget sensitivity run described in Section~\ref{sec:limitations} (Gemini 2.5 Pro, all 60 Track~A tasks at a 16{,}384-token cap, 2026-07-29), which cost \$3.43 and is likewise excluded from the \$51.08: its results appear only in that limitation's discussion and in no table. Total program spend across all three activities is \$62.69. Given the recorded container digest and a specific submission's RTL text, re-scoring that submission is byte-level reproducible; this reproducibility claim applies to the scoring pipeline, not to re-invoking the models themselves, whose sampling is not deterministic (Appendix~\ref{app:variance} quantifies the resulting run-to-run variance).

For Track A (Table~\ref{tab:tracka}), the models are ranked by GateTruth Score, defined in Section~\ref{sec:design} as Pass@1 multiplied by normalized mean PPA; equivalently, because failed tasks contribute a task score of exactly zero, this is the mean capped-PPA task score across all 60 tasks on a 0--100 scale, where the human-authored reference implementation is normalized to $100/1.5 = 66.\overline{6}$ (displayed as 66.7). The resulting hierarchy is: Opus (46.91), Sonnet (45.03), Haiku (33.58), GPT-5-mini (33.44), GPT-5 (31.64), Gemini 2.5 Pro (31.06), and Llama 4 Maverick (25.52). These are single-run figures, and Opus and Sonnet sit within run-to-run variance of one another at the top of the table (Appendix~\ref{app:variance}); a clear gap then separates this frontier pair from the remaining models. The correctness-gate pass@1 metric exhibits a corresponding decline: Opus 42/60, Sonnet 41/60, Haiku 31/60, GPT-5-mini 30/60, GPT-5 and Gemini 2.5 Pro tied at 28/60, and Llama 4 Maverick 23/60. Broken out by tier, pass@1 aggregated across all seven models is 104/140 (74.3\%) on Tier~1, 68/175 (38.9\%) on Tier~2, and 51/105 (48.6\%) on Tier~3 --- Tier~2's protocol and datapath components are, in aggregate, harder for these models than the ostensibly more complex Tier~3 arithmetic and pipeline designs, a non-monotonicity we report rather than smooth over.

A critical observation from this data concerns what actually drives the 30-point gap between the top score (46.91) and the human-reference normalization point (66.7): it is overwhelmingly a correctness-gate effect, not a physical-quality shortfall. Computing mean PPA only over each model's \emph{passed} tasks --- the same quantity Table~\ref{tab:tracka}'s composite score already weights by pass@1, made explicit here --- gives Opus 1.005, Sonnet 0.989, Haiku 0.975, GPT-5-mini 1.003, GPT-5 1.017, Llama 4 Maverick 0.999, and Gemini 2.5 Pro 1.003: a total spread of 4.2\% across all seven models, with the single highest-scoring-per-passed-task model (GPT-5) tied for second-worst pass@1 in the field. On the tasks a model actually passes, its RTL is on average numerically within 2.5\% of --- and for four models nominally \emph{above} --- the human reference on the PPA axis. We deliberately do not call this a statistical equivalence: we have no per-task variance estimate for the PPA ratio, so ``within 2.5\%'' is an observed numerical spread, not a test result. This comparison also carries a selection effect we do not correct for: each model's mean is taken over a different subset of tasks --- the ones it passed --- so a weak model's mean is computed over easier designs than a strong model's, and the columns are not strictly comparable across models. The clean version of this comparison would restrict to tasks passed by all seven models; we report the per-model means as measured and flag the confound rather than presenting the spread as a controlled result. We traced the degeneracy further: of the 50 Track~A tasks passed by at least one model, 17 (concentrated in Tier~1, plus a handful of narrow Tier~2/3 designs) were passed by every passing model at an identical PPA ratio of exactly 1.000, and 135 of the 211 total passing (model, task) pairs across the whole suite --- 64.0\% --- landed at that same exact 1.000. The most plausible explanation is that for a large share of Track~A, especially simple combinational Tier~1 tasks, any functionally correct RTL synthesizes to a near-identical netlist under our flow, so the PPA axis contributes little or no discriminating signal there; PPA becomes informative mainly on the minority of tasks where correct implementations can differ physically, and decisively so only in the agentic-repair track (Section~\ref{sec:trackb}), where the task is explicitly to improve PPA rather than merely reproduce it. We do not claim state-of-the-art models forfeit physical quality relative to a human implementation --- our own data says the opposite on the tasks where a comparison is even possible. Furthermore, two structural insights emerge regarding the evaluation paradigm. First, inference cost and physical quality are decoupled in specific, checkable ways rather than uniformly: Gemini 2.5 Pro is simultaneously the most expensive model evaluated (\$2.16, above Opus's \$1.58) and the lowest-scoring (17.83) at the common token budget --- though the sensitivity result in Section~\ref{sec:limitations} shows most of that scoring gap closes at a higher budget, so this particular inversion is the one most contingent on the experimental condition; Llama 4 Maverick, by contrast, is the cheapest model evaluated by a wide margin (\$0.029, roughly 7$\times$ below its next-cheapest competitor, GPT-5-mini at \$0.203, and nearly two orders of magnitude below Gemini) while still placing sixth rather than last; and GPT-5 underperforms relative to its smaller, cheaper counterpart, GPT-5-mini, despite costing roughly $4.6\times$ more per run --- an inversion that does not depend on the token budget in the same way, since both OpenAI models ran under the identical cap. Such inversions remain entirely opaque to conventional pass@k benchmarks. Second, on Track A the aggregate ranking is governed primarily by the correctness gate---how many tasks each model passes---with post-synthesis PPA contributing a small, signed adjustment rather than a reordering: measured only over the tasks a model passes, the capped geometric mean neither reorders the table nor reliably separates the two frontier models, which are statistically indistinguishable on Track A across repeated runs (Appendix~\ref{app:variance}). Single-shot generation therefore separates current models mainly by correctness; the physical-quality axis, while genuinely measured here, becomes a decisive differentiator only in the agentic-repair track (Section~\ref{sec:trackb}), where optimizing a design against an explicit PPA objective is the task itself.
...
\paragraph{Failure-stage taxonomy.} Table~\ref{tab:tracka} reports a score per model but not where failing submissions fail. Across the campaign's 209 failing (model, task) pairs, 38 (18.2\%) produced no extractable submission at all --- the provider returned no parseable module, so no gate was ever reached --- and these are counted as a distinct category rather than attributed to a pipeline stage. Of the remaining 171 pairs that did produce a submission, 82.5\% fail at Stage~0 (lint), 16.4\% at Stage~1 (simulation), and 1.2\% at Stage~2 (formal). Two consequences deserve to be stated plainly rather than left implicit. First, the formal gate discriminates almost nothing: it is the earliest failing stage for 2 of 209 failures campaign-wide, so for practical purposes the correctness verdict is decided by lint and simulation, and the bounded-model-checking layer --- applied to 46 of 60 tasks --- separates essentially no submissions in this campaign. Second, Stages~3--5 (synthesis, static timing, power) are the earliest failing stage for exactly zero submissions: no design that cleared correctness subsequently failed synthesis or timing analysis, so the physical stages act as measurement rather than as a gate, and the \$300 per-run spend cap likewise never bound (Section~\ref{sec:results}). We report all three non-binding controls explicitly rather than presenting them as active safeguards.

\paragraph{The lint gate is doing more work than intended.} Inspecting the Stage-0 lint logs directly, 109 of the 141 lint failures (77.3\%, and 63.7\% of all submissions that failed any gate) contain no hard syntax or semantic error: Verilator's default policy escalates certain warning classes --- predominantly implicit bit-width truncation and expansion, \texttt{WIDTHTRUNC} and \texttt{WIDTHEXPAND} --- to a fatal exit, and our lint invocation (\texttt{verilator -{}-lint-only -{}-sv}, Verilator 5.051, \texttt{rev v5.050-40-g6c20fdb7b}) applies no waivers. The per-model split of these width-only failures is uneven: Llama 4 Maverick 21, Haiku 19, Opus 14, GPT-5-mini 13, Gemini 2.5 Pro 26, Sonnet 11, GPT-5 5. We take the defensible position that lint-clean RTL is a legitimate deliverable standard --- implicit width mismatches are a real and commonly-mandated defect class in industry sign-off flows, and Verilator flags them for that reason --- and therefore do not waive them. But we will not have it both ways: because this single gate accounts for roughly half of all zero scores in the campaign and its firing rate varies more than fourfold across models, Table~\ref{tab:tracka}'s ranking is materially a joint ranking of RTL correctness \emph{and} width-declaration hygiene, and we do not claim it isolates the former. We have not re-simulated the width-only failures with the warning waived, so we cannot report how many would have passed Stage~1; that measurement, and a dual strict/waived leaderboard, are the correct way to settle what share of the ranking each factor contributes, and both are future work rather than results we can claim here. Note also that the escalate-to-fatal behavior is a version-dependent tool default, which is why the exact Verilator revision is pinned above.

\paragraph{A shared output-token budget confounds the comparison, demonstrated by a controlled re-run.} All seven models were sampled under one output-token cap (4096 tokens), applied uniformly. Uniform is not the same as fair. Models that spend output budget on reasoning or commentary before emitting code have less of it left for the code itself, and the campaign's own failure pattern points straight at this: GPT-5 and GPT-5-mini produced no extractable submission on 22/60 and 11/60 tasks respectively, concentrated on the longest Tier-2 and Tier-3 specifications. To test the hypothesis directly we re-ran a single model, Gemini 2.5 Pro, over all 60 Track~A tasks at a 16{,}384-token cap, changing nothing else (same pinned image, same prompts, same scoring). Its pass@1 rose from 16/60 to 28/60 and its GateTruth Score from 17.83 to 31.06 --- a 13-point swing, larger than the measured run-to-run standard deviation of any model we have characterized (Appendix~\ref{app:variance}) and larger than the gap between third and sixth place in Table~\ref{tab:tracka}. \textbf{Table~\ref{tab:tracka} therefore reports all seven models at the common 4096-token condition, and the raised-budget run is reported here as a labeled single-model sensitivity result, not folded into the leaderboard}; mixing the two conditions in one table would make the ranking uninterpretable. We draw the conservative conclusion: the leaderboard is valid as a comparison at a stated budget, and that budget is low enough to be a first-order experimental variable rather than an implementation detail. Re-running all seven models at the higher cap is the measurement that would settle the ranking, and we flag it as required future work rather than reporting a partially-upgraded table.
...
\texttt{t2\_delay\_trigger} 75.0\%, \texttt{t2\_i2c\_slave} 80.0\%, \texttt{t2\_stream\_upsizer}
80.0\%, \texttt{t2\_stream\_downsizer} 88.2\%, \texttt{t2\_axi\_lite\_regfile} 95.2\%, and
\texttt{t2\_spi\_slave} 95.5\% --- a median of 80.0\%, with seven of the nine below the 95\% floor
they now clear at 97.1--100\%. On these ten tasks the objection lands, and the honest statement is
that mutation gating moved them from a median of 80\% to 97--100\%.
...
sits at 80\%, against RTLLM's measured median of 71.2\% across all 44 of its audited designs. The
defensible claim is therefore not that our testbenches are categorically better, but that mutation
gating is what separates an ungated suite (ours at 80\%, RTLLM's at 71.2\%, both in the same broad
...
\paragraph{Ten tasks were passed by no model, and we have not audited them.} Our own audit logic --- that a testbench nothing ever fails is a testbench worth interrogating --- has a mirror image we should apply to ourselves: a task that every model fails is a candidate for being broken, misspecified, or accidentally unsatisfiable, not necessarily a hard task. Ten of the 60 Track~A tasks were passed by none of the seven models: \texttt{t1\_mod\_n\_counter}, \texttt{t2\_i2c\_slave}, \texttt{t2\_spi\_master}, \texttt{t2\_token\_bucket\_limiter}, \texttt{t2\_uart\_rx}, \texttt{t2\_uart\_tx}, \texttt{t3\_aes\_sbox\_datapath}, \texttt{t3\_booth\_multiplier}, \texttt{t3\_lru\_tracker}, and \texttt{t3\_moving\_sum}. Each has a maintainer-signed-off reference implementation that passes the full pipeline by construction, so none is unsatisfiable in the strict sense --- but that establishes only that \emph{a} solution exists, not that the specification communicates it adequately or that the hidden vectors are not over-constrained relative to the prose. We have not performed a per-task solvability review of these ten, and we name them explicitly so that a reader can weight them appropriately rather than discovering the zero-pass set by recomputation.

\paragraph{Tier difficulty is confounded by extraction failures.} Raw per-tier pass rates across the campaign are Tier~1 70.7\%, Tier~2 37.1\%, and Tier~3 44.8\% --- a non-monotonicity in which Tier~2 appears harder than Tier~3. Part of this is an artifact of the no-extraction failures discussed above, which concentrate on the longer Tier-2 and Tier-3 specifications. Excluding pairs where no submission was produced at all, the rates become Tier~1 71.2\%, Tier~2 42.5\%, and Tier~3 52.2\%. The non-monotonicity narrows but does not disappear, so we report it as a real if smaller effect --- plausibly because Tier~2's protocol and handshake tasks demand exact multi-cycle interface conformance, which our hidden vectors check strictly, whereas several Tier-3 datapaths are arithmetically intricate but interface-simple.
...
under the same 4096-token output cap as the main campaign, so they characterize variance at that
condition only.
