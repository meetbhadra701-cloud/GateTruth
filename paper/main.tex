...
testbench failure during simulation --- extending, across all 44 audited designs rather than only
the three M1 pilot tasks, the same compile-vs-behavioral distinction Section~\ref{sec:rigor}
establishes for our own suite.
...
\paragraph{Failure-stage taxonomy, and a genuine extraction bug we found and fixed while producing it.} We did not originally report which stage each failing submission fails at; assembling that breakdown while responding to review uncovered a real defect in our own extraction pipeline, which we fixed and corrected before finalizing this section rather than merely disclosing an unresolved caveat. Our provider-response parser (\texttt{extract\_module\_source}) only stripped a Markdown code fence when it found both an opening and a matching closing marker; a response truncated before the closing fence fell through to a fallback that accepted the entire raw, truncated text --- fence marker included --- as the submission, provided a \texttt{module} keyword appeared anywhere in it. We found that 35 of Gemini 2.5 Pro's then-44 failing submissions had exactly this shape (an opening \texttt{\`{}\`{}\`{}systemverilog} marker with no close), each scoring zero because Verilator rejected the stray marker as an undefined preprocessor directive before ever assessing the RTL itself --- a harness defect, not a finding about the model. We hardened the parser to reject an unterminated fence outright, verified the fix against the real artifact that exposed it and against all seven models' saved submissions (37 affected files total, 35 Gemini plus one Sonnet and one Llama 4 Maverick one-off, all now correctly rejected with zero regressions on any previously-passing submission), and re-scored Gemini 2.5 Pro's full 60-task Track~A suite under the corrected parser and a raised output-token budget. The result: Gemini 2.5 Pro's true pass@1 is 28/60 and its GateTruth Score is 31.06, not the 16/60 and 17.83 originally reported --- the figures used throughout this paper (Table~\ref{tab:tracka}, Section~\ref{sec:results}) are this corrected re-score, not the original buggy run. With the bug fixed, essentially none of the campaign's remaining 197 failing (model, task) pairs are attributable to a design that compiles and simulates but fails a formal property (84.3\% fail at Stage 0 lint, 14.7\% at Stage 1 simulation, 1.0\% at Stage 2 formal). Inspecting the Stage-0 lint logs directly, 74.8\% of the remaining lint failures involve no genuine syntax or semantic error at all: Verilator's default policy escalates certain classes of warning (predominantly implicit bit-width truncation/expansion, \texttt{WIDTHTRUNC}/\texttt{WIDTHEXPAND}) to a fatal exit, and our lint invocation (\texttt{verilator --lint-only --sv}) applies no waivers, so a submission that would simulate correctly can still score zero for a width-safety style issue. Separately, GPT-5 and GPT-5-mini produced no extractable submission at all (as opposed to a submission that failed a gate) for 22/60 and 11/60 tasks respectively, heavily concentrated on the same Tier-2 and Tier-3 tasks for both models, consistent with a reasoning-token-budget interaction on longer specifications rather than a shared extraction defect; we have not root-caused this further, and it does not affect any figure reported in this paper since neither model's original submissions required re-scoring.
